...
function wyz_scripts() {
	...
	wp_localize_script( 'wyz_my_account_js', 'myAccount', array( 'invalidText' => esc_html__( 'Invalid amount', 'wyzi-business-finder' ), 'reduce' => esc_html__( 'points will be reduced from your balance', 'wyzi-business-finder' ), 'pointsAvailable' => get_user_meta( get_current_user_id(), 'points_available', true ), 'exceeds' => esc_html__( 'points exceed your balance', 'wyzi-business-finder' ) ) );
...
function wyz_enqueue_map_script() {
	wp_enqueue_script( 'wyz_maps_on_pages', plugin_dir_url( __FILE__ ) . 'businesses-and-offers/businesses/js/meta-boxes.js', array( 'jquery' ) );
}
...
function wyz_display_map_metaboxes() {
	...
	$wyz_cmb_header_map->add_field( array(
		...
		'desc'    => sprintf( esc_html__( 'Set in case you want the %s from a default location to appear first', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ),
