...
function wyz_add_login_out_cart_item_to_menu( $items, $args ) {
	...
	if ( 'primary' === $args->theme_location/* && 2 == $template_type*/ ) {
		...
		if ( 'on' == wyz_get_option( $pref.'logged-menu-right-link') && 2 == $template_type ){
	...
	if ( 2 != $template_type &&  ( function_exists( 'wyz_get_option' ) && 'header3' != wyz_get_option( 'header-layout' ) ) ) {
		...
		if ( is_user_logged_in() ) {
			...
			$link = '<a href="' . esc_url( home_url( '/user-account/' ) ) . '" class="wyz-button blue icon" title="' . esc_attr__( 'My Account', 'wyzi-business-finder' ) . '">' . $macc_title . '<i class="fa fa-angle-right"></i></a>';
			...
			$link = '<a href="' . home_url( '/signup/?action=login' ) . '" class="wyz-button blue icon" title="' . esc_attr__( 'Sign in', 'wyzi-business-finder' ) . '">' . esc_html__( 'sign in', 'wyzi-business-finder' ) . '<i class="fa fa-angle-right"></i></a>';
...
add_action( 'pre_get_posts', function( $query ){
	...
	if ( ! empty( $query->is_search ) ) {
		if (isset($_GET['post_type']) && in_array(esc_attr($_GET['post_type']), $post_types)) {
			$query->query_vars['post_type'] = (array) esc_attr($_GET['post_type']);
		} else {
			if ( ! in_array( 'wyz_business', $query->query_vars['post_type'] ) ) {
				array_push( $query->query_vars['post_type'], 'wyz_business' );
			}
			if ( ! in_array( 'wyz_business_post', $query->query_vars['post_type'] ) ) {
				array_push( $query->query_vars['post_type'], 'wyz_business_post' );
			}
			if ( ! in_array( 'wyz_offers', $query->query_vars['post_type'] ) ) {
				array_push( $query->query_vars['post_type'], 'wyz_offers' );
			}
			if ( ! in_array( 'wyz_location', $query->query_vars['post_type'] ) ) {
				array_push( $query->query_vars['post_type'], 'wyz_location' );
			}
		}
	}
}, 500, 2 );*/
...
function wyz_show_hide_admin_bar() {
	...
	if ( 'on' == get_option( 'wyz_businesses_hide_admin_bar', 'on' ) && ( 'client' === $user_role || 'business_owner' === $user_role ) ) {
		return false;
	}
	return true;
}
...
function wyz_admin_block_unauthorized_users() {
	...
	if ( function_exists( 'is_user_wcmp_vendor' ) && is_user_wcmp_vendor( $current_user->ID ) && 'on' != get_option( 'wyz_vendor_restrict_backend_access' ) ) { return; }

	if ( ( ! defined( 'DOING_AJAX' )  || ! DOING_AJAX ) && ( 'on' == get_option( 'wyz_businesses_restrict_backend_access' , 'on' ) && ( 'client' === $user_role || 'business_owner' === $user_role ) ) && ( ! filter_input( INPUT_GET, 'action' ) || 'trash' !== filter_input( INPUT_GET, 'action' ) ) ) {
...
function wyz_transfer_business_new_owner( $post_id, $post, $update ) {
	...
	if ( isset( $_POST['wyz-business-owner-meta-box'] ) ) {
		...
		/*if ( ! user_can( $old_owner_id, 'administrator' ) ) {
			update_user_meta( $old_owner_id, 'has_business', false );
			update_user_meta( $old_owner_id, 'business_id', -1 );
		}*/
		...
		update_user_meta( $user_id, 'business_id', $post_id );*/
...
function wyz_product_owner_business_metabox_markup() {
	...
	$curr_owner_bus_id = get_post_meta( $post->ID, 'business_id', true );
	//wp_die("<h2>This is it: $curr_owner_bus_id</h2>");
	if ( '' == $curr_owner_bus_id || 1 > $curr_owner_bus_id ) {
		$curr_owner_bus_id = -1;
	}
...
function wyz_user_greeting_mail( $user_id ) {
	...
	if ( 'client' === $_POST['subscribtion'] ) {
		$message = wyz_get_option( 'user-greeting-mail' );
	} elseif ( 'business_owner' === $_POST['subscribtion'] ) {
...
function wyz_add_post_content($meta_value, $meta, $post_id, $post){
	...
	if ( $meta == 'wyzi_listing_sticky' ) {
		...
		if ( in_array( $post_id, $featured_posts ) )
			return '<div class="sticky-notice featured-banner"><span>' . esc_html__( 'FEATURED', 'wyzi-business-finder' ) .'</span></div>';
		return '';
	}
	...
	/*if ( 'wyzi_listing_open_close' == $meta) {
