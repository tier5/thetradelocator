	...
	<form name="customerChoices" action="" id="customerChoices" class="bookedClearFix">
		<div class="field">
			<span class="checkbox-radio-block">
				...
				<label for="customer_new"><?php esc_html_e('New customer','wyzi-business-finder'); ?></label>
			</span>
		</div>
		...
		<div class="field">
			<span class="checkbox-radio-block">
				...
				<label for="customer_current"><?php esc_html_e('Current customer','wyzi-business-finder'); ?></label>
			</span>
		</div>
	</form>
	...
	<div class="condition-block customer_choice" id="condition-current">
		...
		<form id="ajaxlogin" action="" method="post" data-date="<?php echo $first_booking['date']; ?>" data-title="<?php echo $first_booking['title']; ?>" data-timeslot="<?php echo $first_booking['timeslot']; ?>" data-calendar-id="<?php echo $first_booking['calendar_id']; ?>">
			<div class="cf-block">
				...
				<?php if ( !is_multisite() ): ?>
				<a href="#" class="booked-forgot-password"><?php esc_html_e( 'I forgot my password.', 'wyzi-business-finder' ); ?></a>
				<?php endif; ?>
				
			</div>
			
			<div class="field">
				<input name="submit" type="submit" class="button button-primary" value="<?php esc_html_e('Sign in', 'wyzi-business-finder') ?>">
		...
		<?php if ( !is_multisite() ): ?>
			<form id="ajaxforgot" action="" method="post">
				...
				<div class="field">
					<input name="submit_forgot" type="submit" class="button button-primary" value="<?php esc_html_e('Reset Password', 'wyzi-business-finder') ?>">
					<button class="booked-forgot-goback button"><?php esc_html_e('Go Back','wyzi-business-finder'); ?></button>
				</div>
			</form>
		<?php endif; ?>
...
<div class="condition-block customer_choice default" id="condition-new">
	<form action="" method="post" id="newAppointmentForm">
		...
		if ( $is_user_logged_in && ! $error_message ) {
			$msg = sprintf( _n( 'You are about to request an appointment for %s.', 'You are about to request appointments for %s.', $total_appts, 'wyzi-business-finder' ), '<em>' . booked_get_name( $booked_current_user->ID ) . '</em>' ) . ' ' . _n( 'Please review and confirm that you would like to request the following appointment:', 'Please review and confirm that you would like to request the following appointments:', $total_appts, 'wyzi-business-finder' );
			echo wpautop( $msg );
		}

		// If there aren't any errors, and the user isn't logged in
		if ( ! $is_user_logged_in && ! $error_message ) {
			$msg = _n( 'Please confirm that you would like to request the following appointment:', 'Please confirm that you would like to request the following appointments:', $total_appts, 'wyzi-business-finder' );
			echo wpautop( $msg );
		}
		...
		<?php if ( ! $is_user_logged_in && ! $error_message && class_exists('ReallySimpleCaptcha') ) : ?>
			...
			?>
			<p class="captcha">
				<label for="captcha_code"><?php esc_html_e('Please enter the following text:','wyzi-business-finder'); ?></label>
		...
		<div class="field">
			<?php if ( $error_message ): ?>
				<button class="cancel button"><?php esc_html_e('Okay','wyzi-business-finder'); ?></button>
			<?php else: ?>
				<input type="submit" id="submit-request-appointment" class="button button-primary" value="<?php echo _n( 'Request Appointment', 'Request Appointments', $total_appts, 'wyzi-business-finder' ); ?>">
				<button class="cancel button"><?php esc_html_e('Cancel','wyzi-business-finder'); ?></button>
			<?php endif; ?>
		</div>
	</form>
