...
function booked_render_timeslots($calendar_id = false){
	...
	<table class="booked-timeslots"<?php echo ($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : ''); ?>>
		<thead>
			<tr>
				<?php foreach($day_loop as $key => $day):
					echo '<th data-day="'.$day_loop_english_array[$key].'">'.$day.'<a href="#" class="booked-add-timeslot button">'.esc_html__('Add','wyzi-business-finder').'...</a></th>';
				endforeach; ?>
			</tr>
		</thead>
		<tbody>
				...
				<th colspan="<?php if (!$hide_weekends): ?>7<?php else: ?>5<?php endif; ?>">
					...
					<span class="monthName">
						<?php if ($monthShown != $currentMonth): ?>
							<a href="#" class="backToMonth" data-goto="<?php echo $currentMonth; ?>"><i class="fa fa-arrow-left"></i>&nbsp;&nbsp;&nbsp;<?php esc_html_e('Back to','wyzi-business-finder'); ?> <?php echo date_i18n('F',strtotime($currentMonth)); ?></a>
						<?php endif; ?>
		...
		foreach($todays_defaults as $timeslot => $count):
			...
			*/
			...
			echo '<div class="timeslot bookedClearFix'.($title ? ' has-title ' : '').'">';
				...
				echo '<span class="timeslot-count">';
					...
					*/
					...
					if (!empty($appts_in_this_timeslot)):
						...
						echo '<strong>'.$booked_appts.' '._n('Appointment','Appointments',$booked_appts,'wyzi-business-finder').':</strong>';
						...
						foreach($appts_in_this_timeslot as $appt_id):
							...
							echo '<span class="appt-block" data-appt-id="'.$appt_id.'">';
								...
								if ( apply_filters('booked_admin_show_calendar_buttons', true) ) {
									echo '<a href="#" class="delete"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').'><i class="fa fa-remove"></i></a>'.($status == 'pending' ? '<button data-appt-id="'.$appt_id.'" class="approve button button-primary">'.esc_html__('Approve','wyzi-business-finder').'</button>' : '');
								}
				...
				echo '</span>';
				echo '<span class="timeslot-people"><button data-timeslot="'.$timeslot.'" data-title="'.esc_attr($title).'" data-date="'.$date.'"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').' class="new-appt button"'.(!$spots_available ? ' disabled' : '').'>'.esc_html__('New Appointment','wyzi-business-finder').' ...</button></span>';
			echo '</div>';
...
function booked_render_custom_fields($calendar = false){
	...
	<button class="cfButton button" data-type="single-line-text-label"><i class="fa fa-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Single Line Text','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="paragraph-text-label"><i class="fa fa-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Paragraph Text','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="checkboxes-label"><i class="fa fa-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Checkboxes','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="radio-buttons-label"><i class="fa fa-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Radio Buttons','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="drop-down-label"><i class="fa fa-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Drop Down','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="plain-text-content"><i class="fa fa-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Text Content','wyzi-business-finder'); ?></button>
