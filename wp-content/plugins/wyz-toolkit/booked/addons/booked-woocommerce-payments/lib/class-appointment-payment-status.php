...
class Booked_WC_Appointment_Payment_Status {
	...
	public function set_statuses() {
		...
		if ( $this->order_id && $this->order_id === 'manual' || $this->order_id && $this->order_obj->order->get_status() === 'wc-completed' ) {
			$this->is_paid = true;
			$this->payment_status = 'paid';
			$this->payment_status_text = __('Order Paid', 'booked-woocommerce-payments');
		} elseif ( $this->order_id ) {
			$this->is_paid = $this->order_obj->order->get_status() === 'wc-completed';
