...
abstract class AccountContent {
	...
	public function the_tab(){
		if ( $this->condition ) {
			echo '<li class="'. $this->active . ' ' . $this->link . '" ><a href="#' . $this->link . '" data-toggle="tab">' . $this->tab_title . '</a></li>';
		}
	}
...
class AccountBusiness extends AccountContent {
	...
	public function _active () {
		return $this->is_business_owner && (! isset( $_POST['updateuser'] ) || ( function_exists( 'is_wc_endpoint_url' ) && ! is_wc_endpoint_url() ) ) ;
	}
	...
	public function notifications() {
		...
		if ( isset( $_GET['post_submitted'] ) ) {
			if ( 'off' == get_option( 'wyz_offer_immediate_publish' ) ) {
				...
				$output .= '<div class="wyz-success"><p>' . sprintf( esc_html__( 'Thank you, your new %s is now pending for submission.', 'wyzi-business-finder' ), esc_html( $name ), WYZ_OFFERS_CPT ) . '</p></div>';

			} else {
				$output .= '<div class="wyz-success"><p>' . sprintf( esc_html__( 'Thank you, your new %s has been published successfully.', 'wyzi-business-finder' ), WYZ_OFFERS_CPT ) . '</p></div>';
			}
		} elseif( isset( $_GET['offer_updated'] ) ) {

			// Add notice of submission to our output.
			$output .= '<div class="wyz-success"><p>' . sprintf( esc_html__( 'Thank you, your %s was updated successfully.', 'wyzi-business-finder' ), WYZ_OFFERS_CPT ) . '</p></div>';
		}
		echo $output;
	}

	public function content() {
		echo /*'<div class="section-title col-xs-12 row margin-bottom-50"><h1>' . esc_html__( 'manage my', 'wyzi-business-finder' ) . ' ' . WYZ_BUSINESS_CPT . '</h1></div>' .*/ wyz_business();
	}
}
...
class AccountFavorite extends AccountContent {
	...
	public function content() {
		...
		if ( empty( $favorites ) )
			echo '<div class="wyz-info"><p>' . esc_html__( 'You don\'t have any favorates yet', 'wyzi-business-finder' ) . '</p></div>';
		else {
...
class AccountProducts extends AccountContent {
	...
	public function content() {
		...
		if(current_user_can('dc_vendor') && ('on' == get_option('wyz_allow_front_end_submit','on'))){
			...
			echo '<div id="shop-settings" class="float-right" style="margin-bottom: 10px;"><a class="blue-btn-square" href="'.get_home_url(null,'/user-account/?product_id=1').'">'.esc_html__('Add a Product','wyzi-business-finder').'</a></div>';
	...
	$wyz_sub_pay_woo_on_off = get_option('wyz_sub_pay_woo_on_off','off');
		...
		if ( 'off' == $wyz_sub_pay_woo_on_off ) {
			...
			if(is_user_logged_in() && function_exists('pmpro_hasMembershipLevel') && pmpro_hasMembershipLevel()) {
				...
				echo '<div class="wyz-info"><p>' . sprintf( esc_html__( 'Your Membership Level: %s', 'wyzi-business-finder' ),$current_user->membership_level->name ) . '</p></div>';
			} else {
				echo '<div class="wyz-info"><p>' . sprintf( esc_html__( 'You don\'t have a subscription yet', 'wyzi-business-finder' )) . '</p></div>';
			}
...
add_action( 'wp', 'wyz_init_user_account',9 );
?>