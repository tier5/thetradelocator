...
jQuery(document).ready(function($) {
	...
	if (jQuery('#wyz-cat-select').length) {
		...
		jQuery('#wyz-cat-select').selectize({
			...
			render: {
				option: function(data, escape) {
					return '<div class="option" data-selectable="" data-value="'+escape(data.value)+'" data-id="'+escape(data.id)+'">'+escape(data.text)+'</div>';
				}
			},
			onItemAdd: function(value, item) {
				if(phpInfo.cat_icon_exits == 'yes') {
					var id = (this.options[value]).id;
					if(catIconOptions[id] != undefined)
		...
		function addOption(id) {
			if(phpInfo.cat_icon_exits == 'yes') {
				if(catIconOptions[id] != undefined&&!idExists(id)){
