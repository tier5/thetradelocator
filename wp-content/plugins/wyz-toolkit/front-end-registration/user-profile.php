...
function wyz_user_profile_form_display() {
	...
	<?php } elseif ( isset( $_POST['upgrade-role-yes'] ) && 1 == $_POST['upgrade-role-yes'] ) {
			...
			$user_id = wp_update_user( array( 'ID' => $user_id, 'role' => 'business_owner' ) );
			if ( is_wp_error( $user_id ) ) {
				echo '<div class="wyz-error"><p>' . esc_html__( 'There was an Error', 'wyzi-business-finder' ) . '</p></div>';
			} else {
				echo '<div class="wyz-success"><p>' . esc_html__( 'Profile Updated', 'wyzi-business-finder' ) . '</p></div>';
		...
		<form name="upgrade-role-yes" method="POST">
			...
			<button type="submit" class="wyz-button orange icon"><?php esc_html_e( 'Upgrade', 'wyzi-business-finder' );?> <i class="fa fa-angle-right"></i></button>
		</form>
	...
	<?php
		...
		if ( isset( $update_error ) && count( $update_error ) > 0 ) {
			echo '<div class="wyz-error">' . implode( '', $update_error ) . '</div>';
		}?>
		<div class="section-title text-center margin-bottom-50">
		...
		<form name="upgrade-role" id="upgrade-role" method="POST">
			...
			<button class="wyz-button blue icon" type="submit"><?php esc_html_e( 'Upgrade Account to Business Owner', 'wyzi-business-finder' );?> <i class="fa fa-angle-right"></i></button>
		</form>
...
function profile_form_1( $user_id ) {
	...
	<div class="profile-form text-center col-lg-6 col-lg-offset-3 col-md-8 col-md-offset-2 col-xs-12">
		<form method="post" action="<?php echo esc_url( get_the_permalink() ); ?>">
			...
			<button name="updateuser" id="update-user" type="submit" class="wyz-button orange icon"><?php echo esc_html__( 'update', 'wyzi-business-finder' ); ?> <i class="fa fa-angle-right"></i></button>
...
function profile_form_2($user_id ) {
	...
	<div class="profile-form text-center col-lg-6 col-lg-offset-3 col-md-8 col-md-offset-2 col-xs-12">
		<form method="post" action="<?php echo esc_url( get_the_permalink() ); ?>">
			...
			do_action( 'wyz_edit_user_profile', $user_id ); ?>
			<div class="col-xs-12">
				<button name="updateuser" id="update-user" type="submit" class="action-btn btn-bg-blue btn-rounded"><?php echo esc_html__( 'update', 'wyzi-business-finder' ); ?></button>
			</div>
			<?php wp_nonce_field( 'update-user' ); ?>
			<input name="action" type="hidden" id="action" value="update-user" />
			<input name="updateuser" type="hidden" value="1" />
		...
		<form method="post" >
			...
			<div class="input-box">
				...
				<div class="input-box">
					...
					if ( $points_fee > 0 ) {?>
					<div class="wyz-info"><p><?php esc_html_e( 'Points transfer costs' );?> <b id="points-fee"><?php echo $points_fee . '</b> ' . esc_html__( 'points', 'wyzi-business-finder' );?>.<br/><span id="amount-notif"></span></p></div>
					<?php }?>
				</div>
			</div>
			...
			<button id="update-user" type="submit" class="wyz-button orange icon"><?php echo esc_html__( 'transfer', 'wyzi-business-finder' ); ?> <i class="fa fa-angle-right"></i></button>
		</form>
	</div>
	<?php
}
...
/**
 * Check user input and uodate profile.
 */
function wyz_check_user_update() {
	global $update_error;
	$update_error = array();
	$user_id = get_current_user_id();
	if ( 'POST' == $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['updateuser'] ) ) {
