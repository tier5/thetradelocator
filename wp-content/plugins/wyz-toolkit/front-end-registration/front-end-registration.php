...
function wyz_signup_display() {
	...
	if ( 1 != get_option( 'users_can_register' ) && (! isset( $_GET['action'] ) || 'login' != $_GET['action'] ) ) {
		return '<div class="wyz-error"><p>' . esc_html__( 'User Registration is not enabled', 'wyzi-business-finder' ) . '</p></div>';
	}

	if ( is_user_logged_in() ) 
		return '<div class="wyz-warning"><p>' . esc_html__( 'You are already logged in.', 'wyzi-business-finder' ) . '</p></div>';
	...
	if ( isset( $_GET['reset-pass'] ) && true == $_GET['reset-pass'] ) {
		...
		if ( ! empty( $reset_key ) && ! empty( $user_data ) ) {
			...
			if ( $message && ! wp_mail( $user_email, esc_html__( 'Password Reset Request', 'wyzi-business-finder' ), $message, $headers ) ) {
				echo '<div class="wyz-error"><p>' . esc_html__( 'Sending email failed', 'wyzi-business-finder' ) . '</p></div>';
				exit();
			} else {
				wp_set_password( $new_password, $user_data->ID );
				echo '<div class="wyz-success"><p>' . esc_html__( 'Password reset Complete', 'wyzi-business-finder' ) . '</p></div>';
				echo '<p>' . esc_html__( 'We have sent you an email with your new password', 'wyzi-business-finder' ) . '</p>';
			}
		} else {
			exit( '<div class="wyz-error"><p>' . esc_html__( 'Not a Valid Key', 'wyzi-business-finder' ) . '</p></div>' );
		}
	} else {
		if ( 1 == $template_type ) {
...
function wyz_login_form() {
	if ( ! is_user_logged_in() ) {
		...
		$output = '<div class="wyz-warning"><p>' . esc_html__( 'You are already logged in.', 'wyzi-business-finder' ) . '</p></div>';
	}
	return $output;
}
...
function wyz_reset_pass_form_fields() {
	...
	if ( '' != $mail_success ) {
		echo '<div class="wyz-success"><p>' . $mail_success . '</p></div>';
	}
	if ( 2 == $template_type )
		...
		$submit_button = '<button id="submit" type="submit" class="wyz-button blue icon">' . esc_html__( 'GET NEW PASSWORD', 'wyzi-business-finder' ) . '<i class="fa fa-angle-right"></i></button>';?>
...
<div class="register-form text-center <?php echo $offset;?>col-md-offset-2 col-md-8 col-xs-12">
		...
		<form id="wyz_registration_form" class="wyz-form" method="POST">
			...
			<button id="submit" type="submit" class="wyz-button orange icon" value=""><?php esc_html_e( 'SIGN UP', 'wyzi-business-finder' ); ?> <i class="fa fa-angle-right"></i></button>
			...
			<a href="<?php echo wp_login_url() . '?loginGoogle=1&redirect=' . home_url( '/user-account/' );?>" class="wyz-button blue icon social-login google"><i class="fa fa-google"></i><?php esc_html_e( 'Sign Up with google', 'wyzi-business-finder' );?></a>
			<?php }?>
			</div>
			<p id="have-acc" class="margin-top-15"><?php esc_html_e( 'Already have an account?', 'wyzi-business-finder' );?> <a href="<?php echo esc_url( home_url( '/signup/?action=login' ) );?>" class="link"><?php esc_html_e( 'Login', 'wyzi-business-finder' );?></a></p>
		</form>
	</div>
...
function wyz_login_form_fields() {
	...
	<div class="login-form col-xs-12 fix">
		<form id="wyz_registration_form" class="wyz-form" method="POST">
			...
			<button id="wyz_login_submit" type="submit" class="wyz-button blue icon"><?php esc_html_e( 'SIGN IN', 'wyzi-business-finder' ); ?> <i class="fa fa-angle-right"></i></button>

			<div class="social-login-container">
				<?php if ( '' != get_option( 'wyz_fb_app_id' ) && '' != get_option( 'wyz_fb_app_secret' ) ) {
					echo do_shortcode( '[fbl_login_button]' );
				/*<a href="<?php echo wp_login_url() . '?loginFacebook=1&redirect=' . home_url( '/user-account/' );?>" class="wyz-button blue icon social-login facebook"><i class="fa fa-facebook"></i><?php esc_html_e( 'Sign In with facebook', 'wyzi-business-finder' );?></a>*/
				}
				if ( '' != get_option( 'wyz_google_client_id' ) && '' != get_option( 'wyz_google_client_secret' ) && '' != get_option( 'wyz_google_developer_key' ) ){ ?>
				<a href="<?php echo wp_login_url() . '?loginGoogle=1&redirect=' . home_url( '/user-account/' );?>" class="wyz-button blue icon social-login google"><i class="fa fa-google"></i><?php esc_html_e( 'Sign In with google', 'wyzi-business-finder' );?></a>
				<?php }?>
			</div>
...
function wyz_login_registration_form() {
	...
	<?php wyz_show_error_messages(); ?>
	<!-- <div class="login-reg-image col-md-5 col-md-offset-0 col-sm-8 col-sm-offset-2 col-xs-12">
		<img src="<?php //echo WYZI_PLUGIN_URL . '/templates-and-shortcodes/images/login-reg.png';?>" alt="">
	</div> -->
	...
	<!-- <div class="col-md-7 col-sm-8 col-md-offset-0 col-sm-offset-2 col-xs-12"> -->
		<div class="login-reg-forms">
			...
			<ul class="login-reg-tab-list mb-50">
				<li class="<?php echo $login_active;?>"><a href="#login-form" data-toggle="tab">login</a></li>
				<li class="<?php echo $register_active;?>"><a href="#reg-form" data-toggle="tab">register</a></li>
			</ul>
				...
				<div class="tab-pane <?php echo $login_active;?>" id="login-form">
					<form action="#" class="login-reg-form" method="post">
						<div class="row">
							...
							<div class="col-xs-12">
								<button id="submit" type="submit" class="action-btn btn-bg-blue btn-rounded"><?php esc_html_e( 'SIGN IN', 'wyzi-business-finder' ); ?></button>
				...
				<div class="tab-pane <?php echo $register_active;?>" id="reg-form">
					<form id="wyz_registration_form" class="login-reg-form" method="POST">
						<div class="row">
							...
							<input type="hidden" name="wyz_register_nonce" value="<?php echo esc_attr( wp_create_nonce( 'wyz-register-nonce' ) ); ?>"/>
							<div class="col-xs-12">
								<button id="submit" type="submit" class="action-btn btn-bg-blue btn-rounded"><?php esc_html_e( 'SIGN UP', 'wyzi-business-finder' ); ?></button>
								<?php wyz_this_get_login_social_options();?>
							</div>
							<?php if ( function_exists( 'wyz_get_option' ) && 'on' == wyz_get_option( 'terms-and-cond-on-off' ) ) {?>
...
function wyz_this_get_login_social_options() {
	...
	<div class="social-login-cont">
		<h5><?php esc_html_e( 'Other Login Options', 'wyzi-business-finder' );?></h5>
...
function wyz_send_pass_email( $email, $name, $login ) {
	...
	$message .= $page_url . /*"action=reset_pwd&key=$key&login=" . rawurlencode( $login ) .*/ "\r\n";
...
function wyz_show_error_messages() {
	if ( $codes = wyz_errors()->get_error_codes() ) {
		echo '<div class="wyz-error">';
		// Loop error codes and display errors.
		foreach ( $codes as $code ) {
			$message = wyz_errors()->get_error_message( $code );
			echo '<p>' . esc_html__( 'Error', 'wyzi-business-finder' ) . '</strong>: ' . esc_html( $message ) . '</p>';
		}
		echo '</div>';
	}
}
