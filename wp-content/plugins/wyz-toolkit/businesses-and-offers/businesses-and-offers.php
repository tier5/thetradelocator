...
add_action( 'init', function() {
	...
	if ( isset( $_GET[ WyzQueryVars::AddNewBusiness ] ) ) {
		...
		if ( ! isset( $_POST['points_nonce'] ) || ! wp_verify_nonce( $_POST['points_nonce'], 'points_form_nonce' ) ) {
			$transfer_errors[] = '<div class="wyz-error"><p>' . esc_html__( 'Security Violation', 'wyzi-business-finder' ) . '</p></div>';
		} else {
			if ( isset( $_POST['username-email'] ) ) {
				...
				if ( ! $transfee_user ) {
					$transfer_errors[] = '<div class="wyz-error"><p>' . esc_html__( 'User not found.', 'wyzi-business-finder') . '</p></div>';
				} else {
					if ( ! isset( $_POST['points'] ) || 0 >= $_POST['points'] ) {
						$transfer_errors[] = '<div class="wyz-error"><p>' . esc_html__( 'Please enter a valid amount of points to transfer', 'wyzi-business-finder') . '</p></div>';
					} else {
						...
						if ( $points_available < $points_fee + $points_transfer ) {
							$transfer_errors[] = '<div class="wyz-error"><p>' . esc_html__( 'You don\'t have enough points to finish the transfer.', 'wyzi-business-finder' ) . '</p></div>';
						} else {
				...
				$transfer_errors[] = '<div class="wyz-error"><p>' . esc_html__( 'User field required', 'wyzi-business-finder') . '</p></div>';
			}
		}
	}
}, 10 );
...
function wyz_do_frontend_my_account_display_shortcode( $atts = array() ) {
	...
	if ( ! is_user_logged_in() ) {
		echo '<div class="wyz-warning"><p>You don\'t have the permission to view this page\'s content</p></div>';
		return;
	}
	...
	elseif ( $WYZ_USER_ACCOUNT_TYPE == WyzQueryVars::TransferPoints && 'on' == get_option( 'wyz_businesses_points_transfer' ) ) :
		...
		if( 0 == $points_available ){
			echo '<div class="wyz-error"><p>' . esc_html__( 'You don\'t have any points.', 'wyzi-business-finder' ) . '</p></div>';
		} elseif ( '' == $points_available ||  $points_fee > $points_available ) {
			echo '<div class="wyz-error"><p>' . esc_html__( 'You need to have a minimum of', 'wyzi-business-finder' ) . ' ' . $points_fee . ' ' . esc_html( 'to be able to transfer points.', 'wyzi-business-finder' ) . '</p></div>';
		} elseif ( isset( $GLOBALS['transfer_complete'] ) && $GLOBALS['transfer_complete'] > 0 ) {
			echo '<div class="wyz-success"><p>' . $GLOBALS['transfer_complete'] . esc_html__( ' points transfered successfully.', 'wyzi-business-finder' ) . '</p></div>';
		}else {
	...
	elseif ( $WYZ_USER_ACCOUNT_TYPE == WyzQueryVars::BusinessCalendar ) :
		...
		/*if ( isset( $_POST['wyz_calendar_created'] ) ) {
			echo '<div class="wyz-success"><p>' . esc_html__( 'Calendar created successfully', 'wyzi-business-finder' ) . '</p></div>';
		}*/
		
		$calendar = WyzHelpers::get_user_calendar();
 		if ( ! $calendar ) :
			echo '<div class="wyz-info"><p>' . sprintf( esc_html__( 'You don\'t have a calendar for this %s yet', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
			?>
			<form method="post">
				...
				<input type="submit" class="blue-btn-square" value="<?php esc_html_e( 'Create Calendar', 'wyzi-business-finder' );?>"/>
			</form>
			<?php
		elseif ( current_user_can( 'manage_options' ) ) :
			echo '<div class="wyz-info"><p>' . sprintf( esc_html__( 'Admins can manage calendars from the %sbackend%s.', 'wyzi-business-finder' ), '<a href="' . admin_url('admin.php?page=booked-settings') . '">', '</a>' ) . '</p></div>';
		else :
