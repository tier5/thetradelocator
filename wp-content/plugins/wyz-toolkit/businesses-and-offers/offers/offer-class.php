...
class WyzOffer{
	...
	private static function the_offer_1( $id, $is_arch ) {
		...
		<div id="post-<?php echo $id; ?>" <?php post_class( 'sin-offer-item row' . ( $is_arch ? ' the-offer' : '' ) ); ?>>
			...
			<div class="content <?php if ( 'on' == wyz_get_option( 'resp' ) ) { echo 'col-md-7 col-xs-12'; } else { echo 'col-xs-7'; }?>">
				<div class="head fix">
					...
					<?php }?>
					<div class="text float-left"><h3><?php echo esc_html( $ttl ); ?></h3><h4><?php echo esc_html( $exrpt ); ?></h4></div>
				</div>
				<p><?php echo nl2br( esc_html( $desc ) ); ?></p>
				<a href="<?php echo esc_url( get_post_permalink( $id ) ); ?>" class="wyz-button orange icon"><?php echo sprintf( esc_html__( 'view %s', 'wyzi-business-finder' ), WYZ_OFFERS_CPT );?> <i class="fa fa-angle-right"></i></a>
			</div>
		</div>
	...
	private static function the_offer_2( $id, $is_arch ) {
		...
		$dscnt = get_post_meta( $id, 'wyz_offers_discount', true );?>
<?php /*
		<div class="sin-offer-item row<?php echo ( $is_arch ? ' the-offer' : '' );?>">
			<div class="image <?php if ( 'on' == wyz_get_option( 'resp' ) ) { echo 'col-md-5 col-xs-12'; } else { echo 'col-xs-5'; }?> float-right"><?php echo $image;
				if ( 0 < $dscnt ) { ?>
				<span class="offer-label"><?php esc_html_e( 'DISCOUNT', 'wyzi-business-finder' );?> <?php echo esc_html( $dscnt );?>%</span>
				<?php }?>
	...
	public static function wyz_the_business_all_offers( $business_id ) {
		...
		if ( empty( $offers_ids ) ) {
			echo '<div class="wyz-info"><p>' . sprintf( esc_html__( 'This %s has no %s to display.', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT, WYZ_OFFERS_CPT ) . '</p></div>';
			return;
		}
