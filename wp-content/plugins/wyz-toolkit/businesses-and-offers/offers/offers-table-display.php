...
function wyz_display_offers() {
	...
	<?php else :
		echo '<div class="wyz-info"><p>' . esc_html( get_option( 'wyz_offer_no_offers' ) ) . '</p></div>';
	endif; ?>

	<?php if ( $pending ) : ?>
		<div class="wyz-info"><p><?php echo ( sprintf( esc_html__( 'You can\'t add a new %s while having ones pending for review', 'wyzi-business-finder' ), WYZ_OFFERS_CPT ) );?></p></div>
	<?php else : 

		if (WyzHelpers::wyz_sub_can_bus_owner_do(get_current_user_id(),'wyzi_sub_business_can_create_offers')) {		?>
		
		<!--<a class="blue-btn-square" href="<?php //echo WyzHelpers::add_clear_query_arg( array( WyzQueryVars::EditOffer => true, WyzQueryVars::BusinessId => $_GET[ WyzQueryVars::ManageBusiness ] ) );?>"><?php //echo sprintf( esc_html__( 'Add New %s', 'wyzi-business-finder' ), WYZ_OFFERS_CPT );?></a>-->

		<a class="blue-btn-square" href="<?php echo WyzHelpers::add_clear_query_arg( array( WyzQueryVars::AddNewOffer => true, WyzQueryVars::BusinessId => $_GET[ WyzQueryVars::ManageBusiness ] ) );?>"><?php echo sprintf( esc_html__( 'Add New %s', 'wyzi-business-finder' ), WYZ_OFFERS_CPT );?></a>
	<?php } endif; ?>
