...
function wyz_do_frontend_offers_edit( $atts ) {
				...
				if ( $WYZ_USER_ACCOUNT_TYPE == WyzQueryVars::EditOffer ) {
					if ( $_GET[ WyzQueryVars::EditOffer ] == $curr_id ) {
						...
						*/
						...
						$output = '<div class="section-title col-xs-12 margin-bottom-50"><div class="row"><h1>' . esc_html__( 'Edit', 'wyzi-business-finder' ) . ' ' . WYZ_OFFERS_CPT . '</h1></div></div>';

						// Get any submission errors.
						if ( ( $error = $cmb->prop( 'submission_error' ) ) && is_wp_error( $error ) ) {
							// If there was an error with the submission, add it to our ouput.
							$output .= '<div class="wyz-error"><p>' . $error->get_error_message() . '</p></div>';
						}
	...
	if ( ! isset( $current_post ) ) {
		return '<div class="wyz-info"><p>' . esc_html__( 'No', 'wyzi-business-finder' ) . ' ' . WYZ_OFFERS_CPT . ' ' . esc_html__( 'to display', 'wyzi-business-finder' ) . '</p></div>';
	}
	return $output;
}
