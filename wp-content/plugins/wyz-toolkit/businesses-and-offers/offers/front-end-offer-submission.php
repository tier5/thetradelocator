...
function wyz_display_add_new_offer_form( $atts ) {
	...
	if ( wyz_check_for_pending() ) {
		echo '<div class="wyz-error"><p>' . sprintf( esc_html__( 'You can\'t add a new %s while having ones pending for review', 'wyzi-business-finder' ), WYZ_OFFERS_CPT ) . '</p></div>';
		return;
	}

	if ( ! current_user_can( 'manage_options' ) && get_the_author_meta( 'points_available', $user_id ) < get_option( 'wyz_offer_point_price' ) ) {
		echo '<div class="wyz-error"><p>' . sprintf( esc_html__( 'You don\'t have enough points credit to publish a new %s', 'wyzi-business-finder' ), WYZ_OFFERS_CPT ) . '</p></div>';
		return;
	}

	if ( ! WyzHelpers::wyz_has_business( $user_id ) ) {
		echo '<div class="wyz-error"><p>' . esc_html__( 'You don\'t have a business yet', 'wyzi-business-finder' ) . '</p></div>';
		return;
	}
	...
	*/
	...
	if ( 'on' != get_option( 'wyz_hide_points' ) ) {
		...
		$output = '<div class="wyz-info"><p>' . sprintf( esc_html__( '%s cost: %d points', 'wyzi-business-finder' ), WYZ_OFFERS_CPT, $offers_cost ) . '</p></div>';
	}
...
function wyz_handle_frontend_new_offer_submission_form() {
	...
	if ( ! current_user_can( 'manage_options' ) && $points_available < $registery_price ) {
		return '<div class="wyz-error"><p>' . sprintf( esc_html__( 'You don\'t have enough points credit to publish a new %s', 'wyzi-business-finder' ), WYZ_OFFERS_CPT ) . '</p></div>';
	}

	$bus_id = $_POST['wyz_business_id'];
	if ( ! WyzHelpers::user_owns_business( $bus_id, get_current_user_id() ) )
		return '<div class="wyz-error"><p>' . sprintf( esc_html__( '%s owner verification error', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
