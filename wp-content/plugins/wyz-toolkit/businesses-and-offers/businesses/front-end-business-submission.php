...
function wyz_display_add_new_business_form( $atts ) {
	...
	if ( ! is_user_logged_in() || ! current_user_can( 'publish_businesses' ) ) {
		return '<div class="wyz-error"><p>' . esc_html__( 'you don\'t have the permisiion to view this page\'s content', 'wyzi-business-finder' ) . '</p></div>';
	}
	
	if ( !WyzHelpers::user_can_create_business( $user_id ) ) {
		return '<div class="wyz-warning"><p>' . sprintf( esc_html__( 'You already have The maximum allowed number of %s', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
	}
	...
	*/
	...
	if ( ( $error = $cmb->prop( 'submission_error' ) ) && is_wp_error( $error ) ) {
		...
		$output .= '<div class="wyz-error"><p>' . esc_html__( $error->get_error_message(), 'wyzi-business-finder' ) . '</p></div>';
	}
...
function wyz_handle_frontend_new_business_submission_form() {
	...
	*/
	...
	if ( '' != $email ) {
		...
		WyzHelpers::wyz_mail( $email, 'New Business Submission', $message );
	}
