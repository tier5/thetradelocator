...
jQuery(document).ready(function() {
	...
	//jQuery('#business-tabs').scrollTabs();
...
jQuery(document).ready(function() {
    ...
    function a() {
        jQuery(window).width() < 992 && !e ? (e = !0, jQuery(".business-tab").click(function() {
        ...
        })) : jQuery(window).width() >= 992 && e && (e = !1, jQuery(".business-tab").unbind("click"))
    }
    ...
    jQuery('#business-tabs>li').each(function(){
    	if(!firstElement)firstElement=jQuery(this);
    	lastElement=jQuery(this);
    	totalWidth += jQuery(this).width();
    });
    jQuery('#business-tabs').css({'width':totalWidth});*/
   /* jQuery('#business-tabs>li a i').each(function(){
...
/*    var mouseWheelEvt = function (event) {
	...
	document.getElementsByClassName('wall-tab-list').attachEvent("onmousewheel", mouseWheelEvt);*/

    /*jQuery('#tab-nxt').on('click', function(e){e.preventDefault();
		jQuery('.wall-tab-list').animate({
			scrollRight: 50
		}, 300);
    });*/
    function b(a, b) {
        b && d && (d = !1, jQuery("#loadmoreajaxloader").fadeTo("fast", 1), jQuery.ajax({
            ...
            success: function(a) {
                ...
                } else jQuery("div#loadmoreajaxloader").html('<div class="row-center">'+business.noPostsMsg+'</div>'), jQuery(window).unbind("inview")
            }
        }))
    }
			...
			jQuery.ajax({
				...
				success: function(html) {
					...
					if ('' !== html) {
						...
						jQuery('#loadmoreratingsajaxloader').html('<div class="row-center text-center">'+business.noMoreRatings+'</div>');
    ...
    if (jQuery(window).resize(function() {
        ...
        jQuery('#business-edit-post-cancel').click(function(){
            ...
            jQuery.ajax({
                type: "POST",
                url: ajaxurl,
                data: "action=bus_post_edit_get&nonce=" + ajaxnonce + "&post-id=" + post_id /*+ "&bus-id=" + business.businessId*/,
                success: function(a) {
                    //if(!a){toastr.error("Unhandled Error");return;}
                    DropDn.slideUp('fast');
                    editing=false;
                    a = JSON.parse(a);
					...
					jQuery("#busi_edit_post_submit").on("click", function() {
						...
						if ("" === a && "" === b) toastr.warning(business.emptyPostError);
						    ...
						    jQuery(this).prop("disabled", !0), jQuery(this).addClass("busi_post_submit-dis"), jQuery(this).removeClass("busi_post_submit"), jQuery(this).prop("value", "Updating...");
						    var d = jQuery(this);
						    jQuery.ajax({
						        type: "POST",
						        url: ajaxurl,
            ...
            //if (jQuery(this).html().length > a) {
                ...
                /*jQuery(this).html(b + '<a href="#" class="read-more">' + business.showMore + '</a><div class="more_text" style="display:none;">' + c + '<a href="#" class="show_less" style="display:none;"> ' + business.showLess + '</a></div>'), jQuery(this).find("a.read-more").click(function(a) {
        ...
        jQuery("#business-post-image").click(function(a) {
            ...
            if ("" === a && "" === b) toastr.warning(business.emptyPost);
                ...
                jQuery(this).prop("disabled", !0), jQuery(this).addClass("busi_post_submit-dis"), jQuery(this).removeClass("busi_post_submit"), jQuery(this).prop("value", business.posting + "...");
                var d = jQuery(this);
                jQuery.ajax({
                    type: "POST",
                    url: ajaxurl,
                    ...
                    success: function(a) {
                    	...
                    	d.prop("disabled", !1), d.addClass("busi_post_submit"), d.removeClass("busi_post_submit-dis"), d.prop("value", business.post);
                    }
                })
            }
        });
		var currentTarget, currentCommInput;
		jQuery(".post_footer_comment_btn").live("click", function(event) {
...
var DeleteDropdown = {
        getDropdown: function(a, b, c, d) {
            return this.ID = a, this.x = b, this.y = c, this.comm = d, this.dropdown = '<div class="delete-dropdown list-group" style="display:none; position: absolute; left: ' + (b - 45) + "px; top: " + c + 'px;" ><a class="list-group-item edit-dropdown-a" data-id="' + this.ID + '" >'+business.editPost+'</a><a class="list-group-item delete-dropdown-a" data-id="' + this.ID + '" >'+business.deletePost+'</a><a class="list-group-item comm-dropdown-a" data-comm="' + (this.comm ? "closed" : "open") + '" data-id="' + this.ID + '" >' + (this.comm ? business.disableComments : business.enableComments) + "</a></div>", jQuery.parseHTML(this.dropdown)
        }
    },
    LoginDropdown = {
        getDropdown: function(a, b, c) {
            return this.x = a, this.y = b, this.dropdown = '<div class="login-dropdown list-group'+(c?' post-comm-dropdown':'')+'" style="display:block; position: absolute; '+(c?'right':'left')+': ' + a + "px; top: " + b + 'px;" >' + (!c?'<span>'+business.likePostq+'</span>':'')+'<p>'+business.loginLike+'</p><a class="wyz-button blue icon" href="' + getBaseUrl() + '/signup/?action=login" title="'+business.signin+'"> '+business.signin+'<i class="fa fa-angle-right" aria-hidden="true"></i></a></div>', jQuery.parseHTML(this.dropdown);
        }
    };
