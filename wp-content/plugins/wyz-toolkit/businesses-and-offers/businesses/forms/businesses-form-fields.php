...
function wyz_add_business_form_field( $field, $type_after, $wyz_cmb_businesses ) {
	...
	switch ( $field['type'] ) {
		...
		case 'time':
			...
			if ( 'separator' == $type_after ) {
				if ( $first_page ) {
					$first_page = false;
					$after_time_row  = '<button type="button" class="btn btn-next blue-btn-square">' . esc_html__( 'Next', 'wyzi-business-finder' ) . '</button></fieldset>';
				} else
					$after_time_row  = '<button type="button" class="btn btn-previous blue-btn-square">' . esc_html__( 'Previous', 'wyzi-business-finder' ) . '</button><button type="button" class="btn btn-next blue-btn-square">' . esc_html__( 'Next', 'wyzi-business-finder' ) . '</button></fieldset>';
			}
			elseif ( '' == $type_after ) {
				if ( 1 == $fields_count )
					$after_time_row = '</fieldset>';
				else
					$after_time_row = '<button type="button" class="btn btn-previous blue-btn-square">' . esc_html__( 'Previous', 'wyzi-business-finder' ) . '</button></fieldset>';
			}
		...
		case 'custom':
			...
			if ( 'separator' == $type_after ) {
				if ( $first_page ) {
					$first_page = false;
					$after_custom_row  = '<button type="button" class="btn btn-next blue-btn-square">' . esc_html__( 'Next', 'wyzi-business-finder' ) . '</button></fieldset>';
				} else
					$after_custom_row = '<button type="button" class="btn btn-previous blue-btn-square">' . esc_html__( 'Previous', 'wyzi-business-finder' ) . '</button><button type="button" class="btn btn-next blue-btn-square">' . esc_html__( 'Next', 'wyzi-business-finder' ) . '</button></fieldset>';
			}
			elseif ( '' == $type_after ) {
				if ( 1 == $fields_count )
					$after_custom_row = '</fieldset>';
				else
					$after_custom_row = '<button type="button" class="btn btn-previous blue-btn-square">' . esc_html__( 'Previous', 'wyzi-business-finder' ) . '</button></fieldset>';
			}
	...
	if ( 'time' != $field['type'] && 'custom' != $field['type'] ) {
		...
		if ( 'separator' == $type_after ) {
			if ( $first_page ) {
				$first_page = false;
				$data['after_row'] = '<button type="button" class="btn btn-next blue-btn-square">' . esc_html__( 'Next', 'wyzi-business-finder' ) . '</button></fieldset>';
			} else{
				$data['after_row'] = '<button type="button" class="btn btn-previous blue-btn-square">' . esc_html__( 'Previous', 'wyzi-business-finder' ) . '</button><button type="button" class="btn btn-next blue-btn-square">' . esc_html__( 'Next', 'wyzi-business-finder' ) . '</button></fieldset>';
			}
		}
		elseif ( '' == $type_after ) {
			if ( 1 == $fields_count ){
			...
			elseif(!$first_page){
				$data['after_row'] = '<button type="button" class="btn btn-previous blue-btn-square">' . esc_html__( 'Previous', 'wyzi-business-finder' ) . '</button></fieldset>';
			}
		}
...
function wyz_options_modify_cmb2_metabox_form_format( $form_format, $object_id, $cmb ) {
	...
	if ( 'wyz_frontend_businesses' === $cmb->cmb_id ) {
		...
		return '<form class="cmb-form" method="post" id="%1$s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%2$s">%3$s<div class="submit-wrap"><input type="submit" name="submit-cmb" value="' . $lbl . '" class="blue-btn-square button-primary"></div></form>';
	}
