...
<div class="create-busi-post">
	...
	<form id="business-post-form" class="create-busi-post-form">
		...
		<div class="form-footer fix">
				...
				<input type="button" id="busi_post_submit" class="busi_post_submit action-btn btn-bg-grey btn-hover-blue" value="<?php esc_html_e('Post', 'wyzi-business-finder');?>" />
				<input type="hidden" id="wyz_business_post_nonce" value="<?php echo wp_create_nonce( 'wyz-business-post-nonce' ); ?>"/>
		</div>
	</form>
</div>
<?php } else {
	echo '<div class="wyz-info" style="float: none;"><p>' . esc_html__( 'You don\'t have enough points to publish posts.', 'wyzi-business-finder' ) . '</p></div>';
}?>