...
function wyz_business() {
	...
	if ( ! WyzHelpers::wyz_sub_can_bus_owner_do( $user_id , 'wyzi_sub_can_create_business' ) ){
		return '<div class="wyz-info"><p>' . sprintf( esc_html__( 'You need to subscribe to a plan to be able to create or edit a %s.', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
	}
	...
	if ( ! WyzHelpers::wyz_has_business( $user_id ) ) {
		$return .= '<div class="wyz-info"><p>' . esc_html( get_option( 'wyz_businesses_no_business' ) ) . '</p></div>';
	}

	if ( WyzHelpers::user_can_create_business( $user_id ) ) {
		$return .= '<div class="row-center"><a class="wyz-button blue icon" href="?add-new-business=true">' . sprintf( esc_html__( 'Add New %s', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . ' <i class="fa fa-angle-right" aria-hidden="true"></i></a></div>';
	}
...
function wyz_display_business( $user_businesses ) {
	...
	if ( filter_input( INPUT_GET, 'business_created' ) && ( isset( $user_businesses['pending'][ $_GET[ 'business_created' ] ] ) || isset( $user_businesses['published'][ $_GET[ 'business_created' ] ] ) ) ) {
		...
		if ( isset( $_GET['not_enough_points'] ) ) {
			echo '<div class="wyz-info"><p>' . sprintf( esc_html__( 'You don\'t have enough points to create a %s, so your %s is now pending', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT, WYZ_BUSINESS_CPT ) . '</p></div>';
		} elseif ( 'off' === get_option( 'wyz_businesses_immediate_publish' ) ) { // If option for imediate publish is turned off by the admin.
			echo '<div class="wyz-success"><p>' . sprintf( esc_html__( 'Thank you, your %s is now pending for review.', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
		} elseif ( 'on' === get_option( 'wyz_businesses_immediate_publish' ) ) { // If option for imediate publish is turned on by the admin.
			echo '<div class="wyz-success"><p>' . sprintf( esc_html__( 'Thank you, your new %s has been created successfully.', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
		}
		...
		echo '<div class="wyz-success"><p>' . sprintf( esc_html__( 'Thank you, your new %s info was updated successfully.', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
	}
...
function wyz_display_user_businesses_table( &$user_businesses ) {
	...
	$btn_class = ( 2 == $template_type ? 'action-btn btn-bg-blue btn-hover-grey' : 'btn');
	if(current_user_can('dc_vendor')){
	echo '<div id="shop-settings" class="float-right"><a class="blue-btn-square" href="'.get_home_url(null,'/wcmp/').'">'.esc_html__('Shop Settings','wyzi-business-finder').'</a></div>';
	}
...
function wyz_display_user_business() {
	...
	if ( ! isset( $user_businesses['published'][ $_GET[ WyzQueryVars::ManageBusiness ] ] ) ) {
		echo '<div class="wyz-info"><p>' . sprintf( esc_html__( 'This %s is pending', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
		if ( 'on' == get_option( 'wyz_businesses_immediate_publish' ) && WyzHelpers::wyz_current_user_affords_business_registry() ) {
			echo '<div class="wyz-success"><p>' . esc_html( 'You have enough points to', 'wyzi-business-finder' ) . ' <a href="' . WyzHelpers::add_clear_query_arg( array( 'publish-business' => $_GET[  WyzQueryVars::ManageBusiness  ] ) ) . '">' . esc_html__( 'Publish', 'wyzi-business-finder' ) . '</a> ' . sprintf( esc_html__( 'your %s', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) . '</p></div>';
		}
	}
	?>
	<!--buttons for view, edit and delete business-->
	<div class="business-buttons col-xs-12 row">
		<a href="<?php echo esc_url( get_the_permalink( $_GET[ WyzQueryVars::ManageBusiness ] ) ); ?>" class="busi-btn"><?php echo ( esc_html__( 'View', 'wyzi-business-finder' ) . ' ' . WYZ_BUSINESS_CPT );?></a>
		<a href="<?php echo WyzHelpers::add_clear_query_arg( array( 'edit-business' => $_GET[  WyzQueryVars::ManageBusiness  ] ) )?>" class="busi-btn"><?php echo ( esc_html__( 'Edit', 'wyzi-business-finder' ) . ' ' . WYZ_BUSINESS_CPT );?></a>
		<a href="<?php echo esc_url( get_delete_post_link( $_GET[ WyzQueryVars::ManageBusiness ] ) ); ?>" class="busi-btn" onclick="return confirm( '<?php echo ( sprintf( esc_html__( 'Are you sure you want to delete your %s? This step is irreversible.', 'wyzi-business-finder' ), WYZ_BUSINESS_CPT ) );?>' );"><?php echo ( esc_html__( 'Delete', 'wyzi-business-finder' ) . ' ' . WYZ_BUSINESS_CPT );?></a>
		...
		<!--<a href="<?php //echo home_url( '/wcmp_vendor_orders/' ); ?>" class="busi-btn"><?php //esc_html_e( 'Shop Settings', 'wyzi-business-finder' );?></a>-->
		<?php }?>
	</div>
	<?php
