...
class WyzSingleBusinessData{
	...
	public function the_tabs_content() {
		...
		foreach ( $this->tab_data as $tab ) {
			$this->get_tab_content( $tab['type'], $tab['urlid'], isset( $tab['active'] ) && $tab['active']?'active ':'' );
		}
	}
	...
	public function get_tab_content( $type, $urlid,$active ) {
		switch ( $type ) {
			case 'wall':
	...
	private function the_tab( $condition, $tab ) {
		if ( $condition ) { ?>
			<li class="<?php echo ( isset( $tab['active'] ) && $tab['active']?'active ':'' ). $tab['type'].' '.$tab['cssClass']; ?>"><a id="<?php echo $tab['type'];?>-btn" class="business-tab" href="#<?php echo $tab['urlid'];?>" ><span><?php echo esc_html( $tab['label'] );?></span><?php if( 2 == $this->template_type ) echo '<i class="fa fa-' . $tab['fa'] . '"></i>';?></a></li>
			<?php
		}
	}
	...
	private function the_wall($active,$urlid) {
		...
		<div class="tab-pane <?php echo $active; ?>row" id="<?php echo $urlid;?>">
			...
			<div class="business-post-area <?php echo $this->responsive;?>">
				...
				if ( ! $this->have_posts ) {
					echo '<div id="no-business-posts" class="wyz-info"><p>' . esc_html( get_option( 'wyz_businesses_no_posts' ) ) . '</p></div>';
				...
				<div class="busi-bank-photos">
					...
					if ( $this->have_images ) { ?>
						...
						<?php $l = count( $this->gallery_data['currentImageAttachedFull'] );
							for ( $i = 0; $i < $l; $i++ ) {
								echo '<div class="col-sm-4 col-xs-6"><a class="sin-photo" href="' . $this->gallery_data['currentImageAttachedFull'][ $i ] . '" data-alt="' . pathinfo( $this->gallery_data['currentImageAttachedFull'][ $i ] )['filename'] . '"><img src="' . $this->gallery_data['currentImageAttachedThumb'][ $i ] . '" width="150" height="150"  /></a></div>';
							}
							?>
						</div>
					<?php
					} else {
						echo '<div class="wyz-info"><p>' . esc_html( get_option( 'wyz_businesses_no_images' ) ) . '</p></div>';
					}
					?>
				</div>
			</div>
			...
			<div class="tab-pane <?php echo $active; ?>" id="<?php echo $urlid;?>">
				<div class="business-offers-area <?php echo $this->responsive;?>">
					...
					if ( ! $this->have_ratings ) {
						echo '<div id="no-business-ratings" class="wyz-info"><p>' . get_option( 'wyz_businesses_no_ratings' ) . '</p></div>';
					...
					echo '<div id="loadmoreratingsajaxloader" class="blog-pagination" style="opacity:0;"><div class="loading-spinner"><div class="dot1"></div><div class="dot2"></div></div></div>';?>
				</div>
				<?php if ( $this->template_type == 1 ) {?>
				<!-- Right Sidebar -->
			...
			<div class="tab-pane <?php echo $active; ?>" id="<?php echo $urlid;?>">
				...
				foreach ( $this->custom_form_data as $key => $value ) {
					...
					if ( $value['type'] == 'selectbox' ) {
						...
						echo '<div class="bus-custom-field"><p>' . get_post_meta( $this->business_id, "wyzi_claim_fields_$key", true ) . '</p></div>';
					}
				}?>
				</div>
				<?php $this->template_type == 1 && $this->right_sidebar();?>
	...
	private function check_for_no_active() {
		...
		if ( ! empty( $_GET ) ) {
			...
			$i;
			for ( $i=0; $i<$count; $i++ ) {
				if ( isset( $_GET[ $this->tab_data[ $i ]['urlid'] ] ) ){
					$found = true;
					$this->tab_data[ $i ]['active'] = true;
					break;
				}
			}
			if ( $found ) {
	...
	private function evaluate_attachments() {
		...
		if ( $this->attachments && '' !== $this->attachments && ! empty( $this->attachments ) ) {
			...
			if ( ! is_array( $this->attachments ) ) {
				$temp_thumb = wp_get_attachment_image_src( $this->attachments, 'thumbnail' );
				...
				foreach ( $this->attachments as $attachment ) {
					$temp_thumb = wp_get_attachment_image_src( $attachment, 'thumbnail' );
