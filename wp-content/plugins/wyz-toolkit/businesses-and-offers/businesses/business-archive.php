		...
		<div class="row">
			...
			<div class="wall-collections<?php if ( 'full-width' === wyz_get_option( 'sidebar-layout' ) ) { ?> col-lg-12 col-md-12 col-xs-12"<?php } elseif ( 'on' === wyz_get_option( 'resp' ) ) { ?> col-lg-9 col-md-8 col-xs-12<?php } else { ?> col-xs-8<?php } ?>">
				...
				if ( ! empty( $args ) ) {
					...
					$the_query = WyzHelpers::query_businesses( array( 'paged' => $paged) );
					$post_ids = array();
					if ( $the_query->have_posts() ) :
				...
				if( 0 === $count ) {
					if ( $location < 1 ) {
						echo '<div class="wyz-info"><p>' . esc_html__( 'No Businesses to show', 'wyzi-business-finder' ) . '</p></div>';
					} else {
						echo '<div class="wyz-info"><p>' . esc_html__( 'No Businesses match your search', 'wyzi-business-finder' ) . '</p></div>';
					}
				} else {
					if ( 2 == $template_type ) {
...
function wyz_add_archives_js() {
	...
	if ( ! empty( $temp_loc ) ) {
		...
		if ( '' != $temp_term_id ) {
			$holder = wp_get_attachment_url( get_term_meta( $temp_term_id, "map_icon$template_type", true ) );
